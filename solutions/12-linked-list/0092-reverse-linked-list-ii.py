...
RECOGNITION HINT  (read only AFTER a real attempt)
    Locate the node before position m, reverse exactly n-m+1 nodes, then
    reconnect both ends.
...
class Solution:
    ...
    #  Time  : O(?)      Space : O(?)
    # -----------------------------------------------------------------
    def reverseBetween_brute(self, head: Optional[ListNode], left: int, right: int) -> Optional[ListNode]:
...
PARAM_TYPES = ['ListNode', 'integer', 'integer']
