...
Pattern    : Binary Tree
Tier       : Core
Scheduled  : Mon 11 Jan 2027  (week 18)

RECOGNITION HINT  (read only AFTER a real attempt)
    Add parent pointers to make it an undirected graph, then plain BFS k
    levels out.
...
class Solution:
    ...
    #  Time  : O(?)      Space : O(?)
    # -----------------------------------------------------------------
    def distanceK_brute(self, root: Optional[TreeNode], target: int, k: int) -> List[int]:
...
INPLACE_ARG = None
NODE_BY_VALUE = [1]   # judge sends a value here, the method wants that node
RETURN_AS   = None
